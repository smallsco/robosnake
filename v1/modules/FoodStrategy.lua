local FoodStrategy = {}
FoodStrategy.__index = FoodStrategy
setmetatable( FoodStrategy, {
    __call = function( cls, ... )
        return cls.new( ... )
    end
})
...
-- @return FoodStrategy self
function FoodStrategy.new( gameState )
    local self = setmetatable( {}, FoodStrategy )
    
    self.food = gameState['food']
    ...
    for i = 1, #gameState['snakes'] do
        ...
        if gameState['snakes'][i]['id'] == Util.SNAKE_ID then
            ...
            self.health = gameState['snakes'][i]['health']
        end
    end
...
function FoodStrategy:test()
    
    -- If there's no food on the map, abort
    if #self.food == 0 then
        ngx.log( ngx.DEBUG, 'Abort FoodStrategy - no food on map' )
        return false
    end
    
    -- If I am the smallest snake on the map, proceed
    --[[local smallest = 99999
    for i = 1, #self.snakeLengths do
        if self.snakeLengths[i] < smallest then
            smallest = self.snakeLengths[i]
        end
    end
    if self.snakeLengths[self.me] == smallest then
        ngx.log( ngx.DEBUG, 'I am the smallest snake, need to eat')
        return true
    end]]
    
    -- If I'm not hungry, abort
    if self.health >= HUNGER_THRESHOLD then
        ngx.log( ngx.DEBUG, string.format('Abort FoodStrategy - not hungry (health: %s)', self.health) )
        return false
    end
...
function FoodStrategy:execute( map )
    --[[
    ...
    for i = 1, #self.food do
        ...
        local my_distance = mdist( self.snakeHeads[self.me], self.food[i] )
        
        -- Find distance from each enemy to the target
        local enemy_is_closer = false
        for j = 1, #self.snakeHeads do
            if j ~= self.me then
                local enemy_distance = mdist( self.snakeHeads[j], self.food[i] )
                -- If I'm the closest, add the food to the targets array
        ...
        if not enemy_is_closer then
            table.insert( targets, { my_distance, self.food[i] } )
        end
    end
    
    -- If there's no targets left, we need to try another strategy
    if #targets == 0 then
        ngx.log( ngx.DEBUG, 'Abort FoodStrategy - no available targets' )
        return false
    end
    ...
    ngx.log( ngx.DEBUG, 'Abort FoodStrategy - no available paths' )
    return false
