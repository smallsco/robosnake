...
function util.beginSegment( name )
    
    -- get last segment id
    local last_segment_id = ngx.ctx.segment_stack[1]
    if not last_segment_id then
        last_segment_id = 1
    end
    
    -- push
    local segment_id = newrelic.begin_generic_segment( tonumber( ngx.var.transaction_id ), last_segment_id, name )
    table.insert( ngx.ctx.segment_stack, 1, segment_id )
    
end

function util.endSegment()
    
    -- pop
    local segment_id = table.remove( ngx.ctx.segment_stack, 1 )
    newrelic.end_segment( tonumber( ngx.var.transaction_id ), segment_id )
...
function util.bieberQuote()
    ...
    return bieberquotes[math.random(#bieberquotes)]
end
